...
Docker allows you to build, run and manage containers, which run software as linux proceses in their own isolated universe, independent of the host platform (windows, mac, linux). Containers are lightweight, meaning they can be created and destroyed within seconds. At the core is a simple standardized text file - called a __dockerfile__ - which contains a set of instructions on how to __build an image__ in a standardized and reproducible way. An image is a template that can be shared and has all the software specified by the dockerfile installed. From an image an arbitrary amount of __containers can be run__.
